...
router.post('/place', async (req, res) => {
    ...
    try {
        ...
        if (price < requiredMin) {
            return res.status(400).json({ error: `Bid too low. Minimum allowed: $${requiredMin}` });
        }
        ...
        const minPerCategory = auction.min_player_per_tier || {};
        const basePricePerCategory = auction.base_price_per_tier || {};
        const totalSlotsRemaining = 11 - totalBought;

        // 🔍 Count how many more players team still needs in each category
        const mandatoryPrices = [];
        for (const [cat, minCount] of Object.entries(minPerCategory)) {
            const current = categoryCounts[cat] || 0;
            const stillNeed = Math.max(minCount - current, 0);
        ...
        const totalFutureNeed = [...mandatoryPrices, ...optionalPrices].reduce((a, b) => a + b, 0);
        const maxAllowed = team.budget_remaining - totalFutureNeed;

        if (price > maxAllowed) {
            return res.status(400).json({
                error: `Bid exceeds safe limit. You need to save at least $${totalFutureNeed} for remaining slots.`
            });
        }
        ...
        return res.status(200).json({ message: "✅ Bid placed successfully", bid: newBid });
