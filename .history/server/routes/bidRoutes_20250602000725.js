...
router.get('/maxSafeBid', async (req, res) => {
    ...
    try {
        ...
        // Count how many players bought per category
        const categoryCounts = {};
        ...
        // Get fixed required player count per category
        const requiredPerCategory = {};
        const basePricePerCategory = {};
        for (const cat of allCategories) {
            requiredPerCategory[cat.name] = cat.min_players;  // fixed requirement
            basePricePerCategory[cat.name] = cat.base_price;
        }

        // Compute cost for remaining category slots
        let totalReserved = 0;
        for (const [cat, required] of Object.entries(requiredPerCategory)) {
            const bought = categoryCounts[cat] || 0;
            const needed = Math.max(required - bought, 0);
            totalReserved += needed * (basePricePerCategory[cat] || 500);
        }

        // Final safe bid = whatever is left after fulfilling mandatory category slots
        const maxSafeBid = Math.max(team.budget_remaining - totalReserved, 0);
