...
router.get('/maxSafeBid', async (req, res) => {
    ...
    try {
        ...
        // 2. Get already bought players (with category)
        const players = await Player.find({ team_id, is_sold: true }).populate('category');

        // 3. Get all categories for the auction
        const categories = await Category.find({ auction_id });

        // 4. Count how many players already bought per category
        const categoryCounts = {};
        ...
        // 5. Build requirement map and base prices
        const requiredPerCategory = {};
        const basePricePerCategory = {};
        for (const cat of categories) {
            requiredPerCategory[cat.name] = cat.min_players; // fixed count per category
            basePricePerCategory[cat.name] = cat.base_price;
        }

        // 6. Compute total reserved funds for unfinished categories
        let totalReserved = 0;
        for (const [catName, required] of Object.entries(requiredPerCategory)) {
            const alreadyBought = categoryCounts[catName] || 0;
            const remaining = Math.max(required - alreadyBought, 0);
            totalReserved += remaining * (basePricePerCategory[catName] || 500);
        }

        // 7. Calculate safe bid
        const maxSafeBid = Math.max(0, Math.min(team.budget_remaining, team.budget_remaining - totalReserved));

        return res.status(200).json({ maxSafeBid });
    } catch (err) {
        console.error("❌ Error in /maxSafeBid:", err.message);
