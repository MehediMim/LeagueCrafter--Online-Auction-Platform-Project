...
router.get('/maxSafeBid', async (req, res) => {
    ...
    try {
        ...
        for (const [cat, required] of Object.entries(requiredPerCategory)) {
            ...
            const unitPrice = basePricePerCategory[cat] || 500;
            totalReserved += stillNeeded * unitPrice;
        }

        // ✅ Core fix — strictly bound safe bid by budget_remaining
        const safeBid = team.budget_remaining - totalReserved;
        const maxSafeBid = Math.max(0, Math.min(safeBid, team.budget_remaining));
