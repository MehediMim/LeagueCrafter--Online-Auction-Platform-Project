...
router.post("/upload-xlsx", upload.single("file"), async (req, res) => {
    ...
    console.log("📦 auction_id:", req.body.auction_id);
    console.log("📎 Uploaded File:", req.file?.originalname || "No file");
    ...
    try {
        ...
        for (const entry of data) {
            ...
            if (!Name || !categoryName) {
                console.warn("⚠️ Skipping invalid row (missing Name/Category)");
                continue;
            }

            const category = await Category.findOne({ name: categoryName, auction_id });
            if (!category) {
                console.warn(`⚠️ Category "${categoryName}" not found for auction ${auction_id}`);
                continue;
            }
        ...
        res.json({ message: "✅ All players added from Excel successfully." });
    } catch (err) {
        console.error("❌ Excel upload failed:", err);
        res.status(500).json({ error: "Internal server error while processing Excel." });
    }
});
