...
router.put('/update/:id', async (req, res) => {
    ...
    try {
        ...
        // Prevent reassignment and double deduction
        if (player.is_sold) {
            return res.status(400).json({ error: "Player is already sold. Cannot assign again." });
        }

        if (team.budget_remaining < basePrice) {
            return res.status(400).json({ error: "Not enough budget to assign this player." });
        }

        // Deduct base price
        const totalRemainingBudget = team.budget_remaining;

        await team.save();

        // Mark player as sold and assign
