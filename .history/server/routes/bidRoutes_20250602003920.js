...
router.get('/maxSafeBid', async (req, res) => {
    ...
    try {
        ...
        const slotsRemaining = totalRequired - totalBought;
        ...
        // Get required and base price per category
        const requiredPerCategory = {};
        const basePricePerCategory = {};
        ...
        // Step 1: Reserve funds for category-required players
        let totalReserved = 0;
        const remainingBreakdown = {};

        for (const [cat, required] of Object.entries(requiredPerCategory)) {
            const bought = categoryCounts[cat] || 0;
            const needed = Math.max(required - bought, 0);
            const base = basePricePerCategory[cat] || 500;
            remainingBreakdown[cat] = needed;
            totalReserved += needed * base;
        }

        // Step 2: Fill remaining optional slots with cheapest available
        const alreadyCounted = Object.values(remainingBreakdown).reduce((a, b) => a + b, 0);
        const optionalSlots = slotsRemaining - alreadyCounted;

        const cheapestPrice = Math.min(...Object.values(basePricePerCategory));
        totalReserved += Math.max(0, optionalSlots) * cheapestPrice;

        // Step 3: Safe max bid
        const maxSafeBid = Math.max(0, Math.min(team.budget_remaining - totalReserved, team.budget_remaining));

        return res.status(200).json({
            team: team.name,
            budget_remaining: team.budget_remaining,
            players_bought: totalBought,
            reserved_for_remaining_slots: totalReserved,
            remaining_per_category: remainingBreakdown,
            safe_optional_slots: optionalSlots,
            maxSafeBid
        });
    } catch (err) {
