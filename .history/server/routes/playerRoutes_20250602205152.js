...
router.post("/upload-xlsx", upload.single("file"), async (req, res) => {
    ...
    try {
        ...
        for (const entry of data) {
            ...
            const categoryName = entry["Category"]?.trim(); // fix typo from "Catergory"
            const rawDob = entry["Date of birth "]; // space included
            const role = entry["Playing position "]?.trim() || "";
            const photo_url = entry["Photo"] || "";
...
router.put('/update/:id', async (req, res) => {
    ...
    try {
        ...
        // Deduct budget and update player
        // team.budget_remaining -= finalPrice;
        await team.save();
