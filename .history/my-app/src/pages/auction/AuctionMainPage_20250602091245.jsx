...
export default function AuctionMainPage() {
    ...
    useEffect(() => {
        ...
        if (auctionId) {
            fetchCurrentPlayer();
            intervalId = setInterval(fetchCurrentPlayer, 1000);
        }
    ...
    useEffect(() => {
        ...
        if (auctionId) {
            ...
            intervalId = setInterval(fetchCurrentPlayer, 1000); // 🔁 keep polling
        }
    ...
    return (
        <ul className="space-y-2">
            {bids.map((bid, index) => (
                ...
                >
                    <div className="z-10 text-black absolute w-40 rounded-full overflow-hidden  flex-shrink-0">
                        <img
                    ...
                    <div className="z-10">
                        <p className="font-bold font-rubik text-2xl text-black">{bid.team_id?.name || "Unnamed Team"}</p>
                        <p className="text-sm text-black">{new Date(bid.timestamp).toLocaleTimeString()}</p>
                    </div>
                    <span className="text-black font-bold text-4xl">${bid.price}</span>
                </li>
            ))}
        </ul>
...
function BudgetList({ auctionId }) {
    ...
    useEffect(() => {
        ...
        if (auctionId) {
            fetchBudgets();
            const interval = setInterval(fetchBudgets, 1000);
            return () => clearInterval(interval);
        }
    }, [auctionId]);
            ...
            <div>
                <ul className="space-y-2 p-4 ">
                    {budgets.map(team => (
                        <li
                            key={team._id}
                            className="cursor-pointer px-2 py-1 rounded text-black font-semibold"
                        ...
                        >
                            <div className="flex justify-between items-center text-black">
                                <div>
                                    <div>{team.name}</div>
                                    <div className="text-sm text-black">Safe Max Bid: ${safeBids[team._id] ?? '...'}</div>
                                </div>
                                <span className="text-lg font-bold">${team.remaining}</span>
                            </div>
                        </li>
...
function TeamModal({ team, auctionId, onClose }) {
    ...
    return (
        <div className="fixed inset-0 bg-white bg-opacity-60 flex justify-center items-center z-50">
            <div ref={hiddenPrintRef} className="bg-gray-900 text-white p-6 rounded-xl w-full max-w-2xl relative  max-h-[90vh] overflow-y-auto">
                {/* <div  className="bg-gray-900 text-white p-6 rounded-xl w-full max-w-2xl relative max-h-[90vh] overflow-y-auto"> */}
                <button
                    onClick={onClose}
                    className="absolute w-10 h-10 flex justify-center items-center top-2 right-2 text-white hover:bg-white hover:text-black text-2xl"
                >
                    &times;
                </button>
                ...
                <button
                    onClick={() => {
                        ...
                        }).from(hiddenPrintRef.current).save();
                    }}
                    className="absolute w-10 h-10 flex justify-center items-center top-2 right-20 text-white hover:bg-white hover:text-black text-2xl"
                >
                    📄
                </button>
                                    ...
                                    <div key={role} className="mb-6">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            {group.map(player => (
                                                <div
                                                    key={player._id}
                                                    className="rounded-lg  text-black shadow-md flex items-center"
            ...
            <div className="hidden">
                <div ref={hiddenPrintRef} className="p-6 w-[600px]">
                    <h2 className="text-2xl font-bold mb-4">{team.name}</h2>
                    <p><strong>Remaining Budget:</strong> ${team.remaining}</p>
                                ...
                                <ul>
                                    {group.map(player => (
                                        <li key={player._id} style={{ marginBottom: '10px' }}>
                                            <strong>{player.name}</strong> – ${player.price} – {player.category?.name}
                                        </li>
                                    ))}
                                </ul>
                            </div>
