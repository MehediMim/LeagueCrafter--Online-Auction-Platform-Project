                ...
                <div className="space-y-10">
                    {Object.entries(
                        players.reduce((acc, player) => {
                            const category = player.category?.name || 'Uncategorized';
                            const role = player.role || 'Unassigned';

                            if (!acc[category]) acc[category] = {};
                            if (!acc[category][role]) acc[category][role] = [];
                            acc[category][role].push(player);

                            return acc;
                        }, {})
                    ).map(([categoryName, roles]) => (
                        <div key={categoryName}>
                            <h2 className="text-3xl font-bold text-yellow-400 mb-4">{categoryName}</h2>
                            {Object.entries(roles).map(([roleName, playersInRole]) => (
                                <div key={roleName} className="mb-6">
                                    <h3 className="text-2xl font-semibold text-cyan-300 mb-2">{roleName}</h3>
                                        ...
                                        {playersInRole.map((player) => (
                                            ...
                                            >
                                                ...
                                                <div className="z-10 h-auto bg-gray-900 bg-opacity-70 rounded-b-xl rounded-xl overflow-hidden">
                                                    ...
                                                    <p className="font-bebas relative h-auto text-3xl flex p-2 bg-gray-900 justify-center bg-opacity-50 items-center">
