                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {players.map((player) => (
                        ...
                        >
                            ...
                            <div className=' z-10  h-auto  bg-gray-900 bg-opacity-70  rounded-b-xl rounded-xl'>
                                <h2 className="text-xl font-semibold font-bebas p-2 truncate">{player.name}</h2>
                                <div>
                                    <p className='font-bebas relative h-auto text-3xl flex px-2 bg-gray-900  rounded-t-xl bg-opacity-50 items-center'>{calculateAge(player.dob)}<div className='text-lg ml-1'> years</div></p>
                                </div>
                                <div className='w-full bg-black flex h-auto justify-start items-center  rounded-b-xl px-2 pt-2 bg-gray-900'><p className='text-xl font-raleway bg-slate-700'>{player.role}</p></div>
                                {/* <p className='text-sm'>{player.category?.name || 'Uncategorized'}</p> */}
                                <div className='bg-gray-900 w-full'>
                                    <button
                                        onClick={() => handleDelete(player._id)}
                                        className="hidden group-hover:block  p-2 bg-red-600 text-white text-sm px-4 py-1 rounded hover:bg-red-700 transition"
                                    >
                                        Delete
                                    </button>
                                </div>
