    ...
    return (
        <ul className="space-y-2">
            {bids.map((bid, index) => (
                <li
                    key={index}
                    className="border p-2 relative rounded-xl shadow flex justify-between items-center"
                    style={{ backgroundColor: bid.team_id?.color_code || '#1f2937' }} // fallback to Tailwind gray-800
                >
                    <div className="w-12 text-black absolute h-12 rounded-full overflow-hidden bg-white flex-shrink-0">
                        <img
