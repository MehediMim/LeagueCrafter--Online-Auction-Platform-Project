    ...
    return (
        <ul className="space-y-2">
            {bids.map((bid, index) => (
                ...
                >
                    <div>
                        <p className="font-bold text-black">{bid.team_id?.name || "Unnamed Team"}</p>
                        <p className="text-sm text-gray-200">{new Date(bid.timestamp).toLocaleTimeString()}</p>
                    </div>
                    <span className="text-yellow-200 font-bold text-lg">${bid.price}</span>
                </li>
            ))}
        </ul>
