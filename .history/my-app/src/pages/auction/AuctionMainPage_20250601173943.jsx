    ...
    return (
        <ul className="space-y-2">
            {bids.map((bid, index) => (
                <li
                    key={index}
                    className="border p-2 rounded-xl shadow flex justify-between items-center"
...
function BudgetList({ auctionId }) {
    ...
    return (
        <div className="ml-4 w-64 bg-gray-800 p-4 rounded-xl border border-gray-700">
            <h3 className="text-xl font-bold w-full bg-black mb-3 text-white text-center">💰 Budgets</h3>
            <ul className="space-y-2">
                {budgets.map(team => (
                    <li
                        key={team._id}
                    ...
                    <ul className="divide-y">
                        {players.map(player => (
                            <li key={player._id} className="py-2">
                                <div className="flex justify-between">
                                    <div>
                                        <p className="font-semibold">{player.name} <span className="text-sm text-gray-500">({player.role})</span></p>
                                        <p className="text-sm text-gray-600">{player.category?.name} – ${player.price}</p>
                                    </div>
                                </div>
                            </li>
                        ))}
