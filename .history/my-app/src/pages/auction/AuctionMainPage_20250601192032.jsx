    ...
    return (
        <ul className="space-y-2">
            {bids.map((bid, index) => (
                <li
                    key={index}
                    className="border p-2 relative overflow-hidden rounded-xl shadow flex justify-between items-center"
                    style={{ backgroundColor: bid.team_id?.color_code || '#1f2937' }} // fallback to Tailwind gray-800
                >
                    <div className=" text-black absolute w-40 rounded-full overflow-hidden bg-opacity-30 flex-shrink-0">
                        <img
