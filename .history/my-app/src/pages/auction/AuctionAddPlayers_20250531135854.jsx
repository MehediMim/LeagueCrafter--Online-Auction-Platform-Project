            ...
            <div className="max-w-7xl mx-auto font-raleway">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {players.map((player) => (
                        <div
                            key={player._id}
                            className="bg-gray-800  h-72 w-52 relative rounded-xl shadow-md flex flex-col justify-end"
                            style={{
                        ...
                        >
                            ...
                            <div className='group z-10 m-2 h-1/2  bg-gray-900 bg-opacity-70 p-2 rounded-b-xl rounded-xl'>
                                <h2 className="text-xl font-semibold font-bebas truncate">{player.name}</h2>
                                <div>
                                    <p className='font-bebas text-3xl flex  items-center'>{calculateAge(player.dob)}<div className='text-lg ml-1'> years</div></p>
                                </div>
                                <div className='w-full flex justify-start items-center rounded-lg'><p className='text-xl font-raleway'>{player.role}</p></div>
                                {/* <p className='text-sm'>{player.category?.name || 'Uncategorized'}</p> */}
                                <button
                                    onClick={() => handleDelete(player._id)}
                                    className="hidden group-valid: mt-2 bg-red-600 text-white text-sm px-4 py-1 rounded hover:bg-red-700 transition"
                                >
                                    Delete
                                </button>
                            </div>
