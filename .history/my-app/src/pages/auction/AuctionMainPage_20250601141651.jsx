...
export default function AuctionMainPage() {
    ...
    const sellPlayer = async () => {
        if (!currentPlayer || !highestBid || !selectedTeamId) {
            setBidStatus("❌ Cannot sell without a team and bid.");
            return;
        }

        try {
            await axios.post(`${API}/auctionDetails/sellPlayer`, {
                auction_id: auctionId,
                player_id: currentPlayer._id,
                team_id: selectedTeamId
            });

            setBidStatus(`✅ ${currentPlayer.name} sold to team successfully.`);
            setCurrentPlayer(null); // Reset after selling
        } catch (err) {
    ...
    const selectRandomUnsoldPlayer = async () => {
        ...
        setIsCustomBid(false); // Reset custom flag on new player
    ...
    const placeBid = async () => {
        ...
        try {
            ...
            const latestHighest = res.data.length > 0 ? res.data[0].price : 0;
                        ...
                        <>
                            ...
                            <div className="mt-6 bg-gray-900 p-4 rounded text-white w-full max-w-md mx-auto">
                                ...
                                <BidList
                                    ...
                                    onHighestBid={(val) => {
                                        const next = val > 0 ? val + bidIncrement : 500;
                                        setHighestBid(val);
                                        if (!isCustomBid) {
                                            setBidAmount(next);
                                        }
    ...
    useEffect(() => {
        const fetchBids = async () => {
            try {
                ...
                if (res.data.length > 0 && onHighestBid) {
                    onHighestBid(res.data[0].price);
                } else if (onHighestBid) {
                    onHighestBid(0);
                }
            } catch (err) {
