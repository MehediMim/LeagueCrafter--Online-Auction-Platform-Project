      ...
      <div className="w-2/3 flex justify-center">
        ...
        {isAuthenticated && (
          <div className="relative ml-3 group">
            {/* Avatar */}
            <div className="h-16 w-16 rounded-full border-amber-600 border-4 overflow-hidden">
              <img
                src={user.picture}
                alt="Profile"
            ...
            {/* Hover Box that stays visible when hovered */}
            <div className="absolute top-20 left-1/2 -translate-x-1/2 w-52 bg-zinc-800 text-white rounded-xl p-4 shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none group-hover:pointer-events-auto transition-all duration-200 z-50">
              <p className="text-lg font-bold font-kanit text-center">{user.name}</p>
              <button
                onClick={() => logout({ returnTo: window.location.origin })}
                className="mt-3 w-full px-4 py-2 text-sm bg-red-600 rounded hover:bg-red-700 transition font-kanit"
              >
                Logout
              </button>
            </div>
